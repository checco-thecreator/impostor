...
io.on('connection', (socket) => {
    ...
    socket.on('submitClue', ({ lobbyCode, clue }) => {
        ...
        if (socket.id !== lobby.players[lobby.turnIndex].id) return;
    ...
    socket.on('playerVote', ({ lobbyCode, votedPlayerId }) => {
        ...
        if (voteCount === lobby.players.length) {
            const tally = {};
            Object.values(lobby.votes).forEach(vote => {
            ...
            io.to(lobbyCode).emit('gameOver', {
                ...
                result,
                word: lobby.word,
                impostors: lobby.players.filter(p => lobby.impostorIds.has(p.id))
            });
        }
    });

    // <-- NUOVO EVENTO: Tentativo dell'impostore -->
    socket.on('impostorGuess', ({ lobbyCode, guess }) => {
        const lobby = lobbies[lobbyCode];
        const player = lobby.players.find(p => p.id === socket.id);
        if (!lobby || lobby.gameState !== 'playing' || !lobby.impostorIds.has(socket.id)) return;
        
        // Confronto senza distinzione tra maiuscole/minuscole e spazi
        ...
        lobby.gameState = 'ended';
        io.to(lobbyCode).emit('gameOver', {
            ejectedPlayer: player, // Mostriamo chi ha tentato il tutto per tutto
            wasImpostor: true, // Tecnicamente vero
            result,
            word: lobby.word,
            impostors: lobby.players.filter(p => lobby.impostorIds.has(p.id))
        });
